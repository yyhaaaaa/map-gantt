<template>
  <div class="gantt">
    ...
    <div class="gantt-table">
      ...
      style="width: 100%">
        <el-table-column
          v-for="(head) in showHeadList"
          :key="head.date || head"
          :align="tableHeaderAlign"
          width="140">>
          <template slot="header" slot-scope="scope">
            <div class="gantt-table__header" v-if="dateType === 'day'">
              {{head}}
            </div>
            <div v-else class="gantt-table__header">
              <span>{{head.date}}</span>
              <span>{{head.weekLabel}}</span>
            </div>
          </template>
          <template slot-scope="scope">
            <div class="gantt-table__cell" :style="getCellHeight(scope.row.maxBlockNum)">
              <template v-for="(mark , index) in scope.row[head]">
                <div class="cell-block color-lump"
                  :class="{'complete-color': mark.state === 'completed'}"
                  :key="index">
                  {{mark.name}}
                </div>
              </template>
              <el-popover
                v-if="scope.row[head] && scope.row[head].length > 3"
                placement="bottom"
                width="288"
                trigger="click">
                <div class="more-workorder-list">
                  <div class="color-lump"
                    :class="{'complete-color': lump.state === 'completed'}"
                    v-for="(lump , index) in scope.row[head].slice(2)" :key="index">
                    {{lump.name}}
                  </div>
                </div>
                <div
                  class="more-workorder"
                  slot="reference">
                  还有{{scope.row[head].length - 2}}个工单…
                </div>
              </el-popover>
            </div>
          </template>
    ...
    handleDayOriginData() {
      this.originData.forEach((staff) => {
        ...
        staff.workOrderList.forEach(workOrder => {
          ...
          // 开始时间和结束时间不在当天的时间范围内，需要特殊处理一下
          for (let i = start; i <= end; i++) {
            if (!mark[this.currentDayList[i]]) {
              mark[this.currentDayList[i]] = []
            }
            mark[this.currentDayList[i]].push({
              name: workOrder.workOrderName,
              id: workOrder.id,
              state: workOrder.state
            })
          }
          // if (!mark[this.currentDayList[start]]) {
          //   mark[this.currentDayList[start]] = []
          // }
          // mark[this.currentDayList[start]].push({
          //   name: workOrder.workOrderName,
          //   id: workOrder.id,
          //   start,
          //   end,
          //   overflowBlock: end - start
          // })
        })
        this.tableData1.push(mark)
      })
      // 计算每一行格子里最多有几块
      this.tableData1.forEach(row => {
        ...
        Object.keys(row).forEach(item => {
          if (row[item].length > row.maxBlockNum) {
            row.maxBlockNum = row[item].length
          }
        })
      })
    },
